...
// Function to send message along with timestamp to primary device
void sendMessage() {
    unsigned long timestamp = millis();
    char message[] = "Hello from secondary!";
    uint8_t payload[sizeof(timestamp) + sizeof(message)];

    // Copy the timestamp to the beginning of the payload
    memcpy(payload, &timestamp, sizeof(timestamp));
    // Copy the message to the payload
    memcpy(payload + sizeof(timestamp), message, sizeof(message));

    // Send the payload to the primary device
